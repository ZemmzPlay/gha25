        ...
        <form class="form-horizontal" method="post" enctype="multipart/form-data">
            ...
            <div class="widget-body">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <input id="first_name" type="text" class="form-control" name="first_name" required value="{{ old('first_name') ? old('first_name') : $member->first_name}}">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
                        <input id="last_name" type="text" class="form-control" name="last_name" required value="{{ old('last_name') ? old('last_name') : $member->last_name}}">
                    </div>
                </div>
                        ...
                        <div class="col-sm-9">
                            <select id="category" name="faculty_category_id" class="form-control">
                                @foreach($categories as $category)
                                    <option value="{{$category->id}}" {{($category->id == $member->faculty_category_id) ? "selected" : ""}}>{{$category->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-9">
                            <select required id="permission_id" name="permission_id[]" class="form-control" multiple="multiple">
                                @foreach($permissions as $permission)
                                    <option value="{{$permission->id}}" {{(in_array($permission->id,$selected_permissions)) ? "selected" : ""}}>{{$permission->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <label for="image_file" style="border:1px solid #ddd; display:inline-block; padding: 3px; margin-left: 15px;">
                        ...
                        @else
                            <div class="img-container-list">
                                <img src="{{asset('images/faculty/'.$member->image_file)}}" id="FacultyMemberImage" style="width: 200px;">
                            </div>
                            <input type="file" name="image_file" id="image_file" style="display: none;">
                        @endif
                    </label>
